...
- Installing OpenCV:
  ...
  ```
- [Optical flow](https://en.wikipedia.org/wiki/Optical_flow) estimation is employed to track moving objects in a video sequence. In this section, we will employ the purely opencv based [sample code](/optical_flow.py) for estimaging the flow using Lucas Kanade Optical Flow approach and Flow Farneback approach. The displays are in the form of streamlines or directional arrows as shown below. \
...
- Hand landmark detection
  - Download the handlandmark detection model:
    ...
    ``` 
  - The [sample code](/hand_landmark.py) employs opencv and mediapipe to detect the human hand and subsequently the finger locations (the tip of thumb and index finger as well as a simple logic to predict if the thumb is pointing up) based on the [finger model](https://developers.google.com/mediapipe/solutions/vision/hand_landmarker) outlined below :
...
- Hand gesture recognition
  - Download the hand gesture recognition model:
    ...
    ``` 
  - The [sample code](/hand_gesture.py) shows a real-time hand gesture recongition task. A sample snapshot of the code result for victory sign is shown below: \
...
- Object detection
  - Download the light weight EfficientDet object detection model:
    ...
    ``` 
  - The [sample code](/obj_detection.py) shows a real-time object detection task.
